...
if __name__ == '__main__':
    ...
    price = md.fetch_historical(period='2y', interval='1d')

    # generate features
    features_df = prepare_features(price)
    ...
    # merge
    df = price.loc[features_df.index, ['Close']]

    # run optimized walk-forward
    equity_curve = walkforward_backtest(df, features_df, label,
                                       train_window=252,
                                       test_window=21,
                                       stop_loss_pct=0.03,
    ...
    for k, v in metrics['summary'].items():
        if isinstance(v, float):
            print(f"{k}: {v:.2%}" if k != 'sharpe_ratio' else f"{k}: {v:.2f}")
        else:
            print(f"{k}: {v}")
    ...
    equity_curve.plot(title='Optimized Walk-Forward Equity Curve')
    plt.ylabel('Account Value ($)')
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('data/equity_optimized.png')
    plt.show()
